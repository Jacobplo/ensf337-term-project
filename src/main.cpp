    ...
    switch(choice) {
      ...
      case 4:
      ...
      
        {
          ...
          // ---- Check if seat is occupied BEFORE calling class function ----
          int col = seat_char - 'A';
          if (sel_flight->getSeats()->isSeatTaken(row, col)) {
      ...
      case 5:
      ...
      
        {
          ...
          // ---- Check if passenger exists BEFORE calling removePassenger() ----
          bool exists = false;
          for (const Passenger &p : *sel_flight->get_passengers()) {
            if (p.get_id() == id) {
              exists = true;
              break;
            }
